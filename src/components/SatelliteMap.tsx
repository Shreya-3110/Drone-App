...
import { MAP_CONFIG } from '../config/dashboard';
...
// Leaflet can't measure its container when hidden (display:none).
// This forces a size recalculation whenever the panel becomes visible.
function InvalidateOnVisible() {
  ...
  useEffect(() => {
    ...
    // Also invalidate immediately in case we're already visible
    map.invalidateSize({ animate: false });
...
export function SatelliteMap({ isConnected }: { isConnected: boolean }) {
  const [position, setPosition] = useState<[number, number]>(MAP_CONFIG.startPosition);
  const [path, setPath] = useState<[number, number][]>([MAP_CONFIG.startPosition]);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setPosition(prev => {
        ...
        setPath(curr => {
          const newPath = [...curr, newPos];
          return newPath.length > MAP_CONFIG.maxPathLength ? newPath.slice(-MAP_CONFIG.maxPathLength) : newPath;
        });
  ...
  return (
    <motion.div
      ...
      role="region"
      aria-label="Satellite map with drone position"
    >
      <div className="absolute top-6 left-6 flex items-center gap-2 z-[400] bg-black/50 px-3 py-1.5 md:px-4 md:py-2 rounded border border-white/10 backdrop-blur-md">
        ...
        <span className={`font-orbitron font-bold tracking-widest text-[10px] md:text-sm shadow-none uppercase ${isConnected ? 'text-[#00FF9C]' : 'text-white/30'}`} aria-live="polite">
