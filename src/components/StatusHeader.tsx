  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 md:gap-4 w-full xl:w-auto overflow-x-auto pb-2 xl:pb-0">
        <select disabled={isConnected} className="bg-black/80 border border-[#00D4FF]/50 text-[#00D4FF] font-orbitron text-xs md:text-sm px-3 md:px-4 py-1.5 md:py-2 rounded-md outline-none focus:border-[#00D4FF] transition-colors disabled:opacity-50 cursor-pointer shadow-[0_0_10px_rgba(0,212,255,0.2)]">
          <option>COM1 (UDP)</option>
          <option>COM3 (USB)</option>
          <option>COM4 (AUTO)</option>
          <option>TCP</option>
        </select>
      ...
      <div className="flex items-center justify-between w-full xl:w-auto xl:justify-end gap-x-3 md:gap-6 text-sm font-orbitron border-t border-white/10 xl:border-none pt-2 xl:pt-0">
        ...
        <div className={`flex gap-2 items-center px-4 py-1.5 rounded-full border ${isConnected ? 'text-[#00FF9C] border-[#00FF9C]/30 bg-[#00FF9C]/10 shadow-[0_0_10px_rgba(0,255,156,0.2)]' : 'text-white/30 border-white/10 bg-white/5'}`}>
        ...
        <button onClick={onToggleMute} className="flex gap-2 items-center text-white/50 hover:text-white transition-colors p-2 bg-white/5 hover:bg-white/10 rounded-full border border-white/10 shrink-0">
